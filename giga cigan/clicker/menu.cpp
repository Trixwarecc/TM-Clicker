...
void c_menu::on_paint( HWND hwnd, int i_width, int i_height )
{
	...
	if ( ImGui::Begin( "TM Clicker", &b_open, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove ) )
	{
		...
		{
			if ( ImGui::BeginTabItem( "TM Clicker" ) )
			{
				ImGui::Text( "BIND" );
				ImGui::Separator();
				...
				ImGui::Checkbox( "Left mouse", &config.clicker.b_enable_left_clicker );
				...
				ImGui::Checkbox( "Right mouse", &config.clicker.b_enable_right_clicker );
				...
				{
					case 0:
						ImGui::Checkbox( "always", &config.clicker.b_only_in_game );
						if ( ImGui::IsItemHovered() )
							ImGui::SetTooltip( "enable this if u want clicker to works .\nThis will be fixed soon dsc.gg/Tmdiscord." );
				...
				if ( ImGui::IsItemHovered() )
					ImGui::SetTooltip( "Best to use lol.\nDsc.gg/Tmdiscord" );

				ImGui::EndTabItem();
			}

			if ( ImGui::BeginTabItem( "misc [useless]" ) )
			{
				ImGui::Text( "Hide" );
				ImGui::Separator();
				...
				if ( show_advanced_debug )
				{
					...
					ImGui::Separator();
					ImGui::Text( "dsc.gg/tmdiscord" );
				}

				ImGui::EndTabItem();
			}

			if ( ImGui::BeginTabItem( "cfgs" ) )
			{
